...
void MenuState::Update(float a_dt)
{
	...
	SpawnFallingObjectsOverTime(a_dt);
}
...
void MenuState::SpawnFallingObjectsOverTime(float a_dt)
{
	m_totalTime += a_dt;
	...
	if (m_totalTime > m_timeLimit)
	{
		...
		int r = (rand() % 2) + 1, randXPos = (rand() % 70) + 1, randSize = (rand() % 5) + 2;
		...
		{
			/*Box* randomBox = new Box(glm::vec2(-randXPos, 65), glm::vec2(0), 0, 10.0f, randSize, randSize,
				RandomColour());*/

			Sphere* randomBall2 = new Sphere(glm::vec2(-randXPos, 65), glm::vec2(0, 0), 10.0f, randSize, RandomColour());
